...
export default function NoticeManager() {
  ...
  // ✅ সেভ ফাংশন (created_by বাদ দেওয়া হয়েছে)
    ...
    <div style={styles.container}>
          ...
          <div style={styles.previewCard}>
            <div style={styles.previewHeader}>
              ...
              <div style={styles.previewActions}>
                {!editingFeatured ? (
                  <button onClick={handleEditFeatured} style={styles.editIconBtn} title="এডিট করুন">✏️</button>
                ) : (
                  <button onClick={() => setEditingFeatured(false)} style={styles.cancelEditBtn}>✕ বাতিল</button>
                )}
              </div>
            </div>
              ...
              {editingFeatured ? (
                <div style={styles.editArea}>
                  ...
                  <div style={styles.editActions}>
                    <button onClick={handleSaveFeatured} disabled={saving} style={styles.saveFeaturedBtn}>
                ...
                <div style={styles.previewContent}>
                  ...
                  <div style={styles.previewMeta}>
                    <span style={styles.previewDate}>📅 {new Date(featuredNotice.created_at).toLocaleDateString('bn-BD')}</span>
      ...
      <div style={styles.header}>
        ...
        <button
          onClick={() => {
            setShowForm(true);
            setEditing(null);
            setFormData({ title: '', message: '', target_role: 'both', target_class: '', is_featured: false });
          }}
          style={styles.addBtn}
        >
          ➕ নতুন নোটিশ
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={styles.switchContainer}>
            <label style={styles.switchLabel2}>
              <input type="checkbox" name="is_featured" checked={formData.is_featured} onChange={handleChange} style={styles.switchInput} />
      ...
      {loading ? (
        ...
        <div style={styles.list}>
          {notices.map((n) => (
            <div key={n.id} style={{ ...styles.item, ...(n.is_featured ? styles.itemFeatured : {}) }}>
              <div style={styles.itemLeft}>
                ...
                {n.is_featured && <span style={styles.featuredBadge}>⭐ হোমপেজে</span>}
              </div>
...
const styles = {
  container: { maxWidth: '900px', margin: '0 auto', fontFamily: "'Hind Siliguri', sans-serif" },
  popup: {
  ...
  previewTitle: {
    ...
    borderBottom: '2px solid #f1f5f9', paddingBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px',
  },
  previewLoading: { textAlign: 'center', color: '#94a3b8', padding: '20px 0' },
  previewCard: { background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' },
  previewHeader: {
    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
    padding: '12px 16px', background: 'white', borderBottom: '1px solid #e2e8f0',
  },
  previewBadge: { fontSize: '13px', fontWeight: '600', color: '#0f172a' },
  previewActions: { display: 'flex', gap: '8px' },
  editIconBtn: { background: '#f1f5f9', border: 'none', padding: '4px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '14px' },
  cancelEditBtn: { background: '#fee2e2', border: 'none', padding: '4px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: '600', color: '#dc2626' },
  previewBody: { padding: '16px' },
  previewContent: {},
  previewText: { fontSize: '15px', color: '#0f172a', margin: '0 0 8px 0', lineHeight: '1.6' },
  previewMeta: { display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#64748b', marginTop: '8px', borderTop: '1px solid #e2e8f0', paddingTop: '8px' },
  previewDate: {}, previewStatus: { fontWeight: '600', color: '#16a34a' },
  previewEmpty: { textAlign: 'center', padding: '30px 20px', background: '#fef3c7', borderRadius: '12px', border: '1px solid #fcd34d' },
  previewEmptyIcon: { fontSize: '32px', display: 'block', marginBottom: '8px' },
  previewEmptyText: { fontSize: '14px', color: '#92400e', margin: 0, lineHeight: '1.6' },
  switchStatus: {
    display: 'flex', alignItems: 'center', gap: '12px', marginTop: '12px',
    padding: '12px 16px', background: '#f1f5f9', borderRadius: '10px',
  ...
  addBtn: { background: '#16a34a', color: 'white', border: 'none', padding: '10px 18px', borderRadius: '10px', fontWeight: '600', cursor: 'pointer', fontSize: '14px' },
  ...
  switchContainer: { display: 'flex', alignItems: 'center', padding: '8px 0' },
  switchLabel2: { display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', position: 'relative' },
  switchInput: { position: 'absolute', opacity: 0, width: 0, height: 0 },
  switchSlider: { width: '44px', height: '24px', background: '#cbd5e1', borderRadius: '12px', transition: '0.3s ease', position: 'relative', display: 'inline-block', flexShrink: 0 },
  switchText: { fontSize: '14px', fontWeight: '500', color: '#334155' },
  ...
  itemFeatured: { borderColor: '#16a34a', borderWidth: '2px', background: '#f0fdf4' },
  itemLeft: { display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', flex: 1 },
  itemRight: { display: 'flex', alignItems: 'center', gap: '6px' },
  badge: { background: '#dbeafe', color: '#2563eb', padding: '2px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '600' },
  badge2: { background: '#f1f5f9', color: '#64748b', padding: '2px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '600' },
  featuredBadge: { background: '#dcfce7', color: '#16a34a', padding: '2px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '700' },
  toggleSwitch: { position: 'relative', display: 'inline-block', width: '40px', height: '22px', flexShrink: 0 },
  toggleInput: { opacity: 0, width: 0, height: 0 },
  toggleSlider: { position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#cbd5e1', transition: '0.3s ease', borderRadius: '22px' },
  ...
  editTextarea: { padding: '10px 14px', borderRadius: '10px', border: '1.5px solid #16a34a', fontSize: '14px', fontFamily: 'inherit', resize: 'vertical', minHeight: '60px', outline: 'none' },
  editActions: { display: 'flex', gap: '10px' },
  saveFeaturedBtn: { background: 'linear-gradient(135deg, #16a34a, #15803d)', color: 'white', border: 'none', padding: '8px 20px', borderRadius: '10px', fontWeight: '600', cursor: 'pointer' },
};
